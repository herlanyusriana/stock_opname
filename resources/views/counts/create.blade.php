@php
    $initialItems = old('items', [
        ['part_id' => '', 'quantity' => 0, 'production_date' => now()->format('Y-m-d'), 'shift' => ''],
    ]);
...
<x-app-layout>
    <div class="space-y-6" x-data="countForm(@js($initialItems))">
        <div class="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            <div>
                <p class="text-sm text-gray-500">Create Count</p>
                <h1 class="text-3xl font-semibold text-gray-900">New Stock Count</h1>
                <p class="text-sm text-gray-500">Capture quantities per part for a specific warehouse location.</p>
            </div>
            <a href="{{ route('counts.index') }}" class="inline-flex items-center gap-2 rounded-xl border border-gray-200 px-4 py-2 text-sm text-gray-600 hover:bg-gray-50">
                Back to Records
            </a>
        </div>
        ...
        <div class="bg-white rounded-3xl border border-gray-100 shadow-sm p-6 space-y-6">
            ...
            <form method="POST" action="{{ route('counts.store') }}" class="space-y-6">
                @csrf
                <div class="grid gap-5 md:grid-cols-2">
                    <div class="space-y-2">
                        <label class="text-sm font-medium text-gray-700">Location</label>
                        <select name="location_id" class="w-full rounded-2xl border border-gray-200 px-4 py-2.5 text-gray-700 focus:border-indigo-500 focus:ring-0" required>
                ...
                <div class="grid gap-5 md:grid-cols-2">
                    ...
                    <div class="space-y-2">
                        ...
                        <input type="text" name="notes" value="{{ old('notes') }}" class="w-full rounded-2xl border border-gray-200 px-4 py-2.5 focus:border-indigo-500 focus:ring-0">
                    </div>
                </div>
                ...
                <div class="space-y-4">
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="text-sm text-gray-500">Add each part counted with its quantity and optional per-item metadata.</p>
                        </div>
                ...
                <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-end">
                    <button type="reset" class="inline-flex items-center justify-center rounded-xl border border-gray-200 px-4 py-2 text-sm text-gray-600 hover:bg-gray-50">
                        Reset
                    </button>
