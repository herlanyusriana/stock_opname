...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body class="font-sans antialiased bg-gray-50 text-gray-900">
        ...
        <div class="min-h-screen flex">
            <aside class="hidden lg:flex lg:flex-col lg:w-64 xl:w-72 bg-white border-r border-gray-100">
                <div class="px-6 py-8">
                    <div class="text-lg font-semibold tracking-tight text-gray-900">
                        Warehouse Tracking System
                    </div>
                    <p class="text-sm text-gray-500 mt-1">Operational Console</p>
                </div>
                <nav class="flex-1 px-4 space-y-1">
                    @foreach ($navLinks as $link)
                        ...
                        <a href="{{ route($link['route'], $link['params'] ?? []) }}"
                           class="flex items-center gap-3 rounded-xl px-4 py-3 text-sm font-medium transition {{ $active ? 'bg-gray-100 text-gray-900 shadow-sm' : 'text-gray-500 hover:bg-gray-50' }}">
                            <span class="inline-flex items-center justify-center rounded-lg bg-gray-100 text-gray-600 p-2">
                ...
                <div class="mt-auto px-6 py-6 border-t border-gray-100 space-y-4">
                    <div class="flex items-center gap-3">
                        <div class="w-10 h-10 rounded-full bg-indigo-100 text-indigo-600 font-semibold flex items-center justify-center">
                    ...
                    <form action="{{ route('logout') }}" method="POST">
                        @csrf
                        <button type="submit" class="w-full inline-flex items-center justify-center gap-2 rounded-xl border border-gray-200 px-4 py-2 text-sm font-medium text-gray-600 hover:bg-gray-50 transition">
                            <svg class="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round">
            ...
            <div class="flex-1 flex flex-col min-h-screen">
                <header class="bg-white border-b border-gray-100 px-6 lg:px-10 py-4 flex items-center justify-between">
                    <div>
                        <p class="text-xs uppercase tracking-[0.2em] text-gray-400">Warehouse Tracking System</p>
                        <h1 class="text-xl font-semibold text-gray-900">@yield('page-title', 'Overview')</h1>
                    </div>
                    <div class="flex items-center gap-3">
                        <div class="hidden sm:flex flex-col text-right">
                            <span class="text-sm font-medium text-gray-900">{{ auth()->user()->name }}</span>
                            <span class="text-xs text-gray-500">{{ ucfirst(str_replace('_', ' ', auth()->user()->role?->value ?? 'user')) }}</span>
                        </div>
                        <div class="w-12 h-12 rounded-full bg-gray-100 flex items-center justify-center border border-gray-200">
                            <svg class="w-6 h-6 text-gray-500" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round">
